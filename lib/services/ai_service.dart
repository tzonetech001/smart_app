...
class AIService {
  ...
  /// - Web/desktop/iOS simulator → localhost:3000
  ...
  Map<String, dynamic> _getLocalPrediction(List<ProductModel> products) {
    double avgEngagement = products.isEmpty
        ? 0
        : products.map((p) => p.engagementScore).reduce((a, b) => a + b) /
            products.length;

    double predictedGrowth =
        avgEngagement > 1000 ? 0.20 : (avgEngagement > 500 ? 0.10 : -0.05);
    ...
    for (var product in products) {
      if (product.engagementScore > 1000 && product.stock < 50) {
        ...
        recommendations.add({
          ...
          'productName': product.productName,
          'recommendation':
              'Consider promotion or discount for ${product.productName}',
          'priority': 'MEDIUM',
        });
      }
    }
    ...
    return {
      ...
      'demandLevel':
          predictedGrowth > 0.15 ? 'HIGH' : (predictedGrowth > 0 ? 'MEDIUM' : 'LOW'),
  ...
  String _analyzeSentimentLocally(String comment) {
    ...
    int positiveCount =
        positiveWords.where((w) => lowerComment.contains(w)).length;
    int negativeCount =
        negativeWords.where((w) => lowerComment.contains(w)).length;
  ...
  Future<List<Map<String, dynamic>>> getMarketTrends(
    ...
    for (int i = 0; i < sortedProducts.length && i < 5; i++) {
      ...
      double trendGrowth = product.engagementScore > 1000
          ? 0.35
          : (product.engagementScore > 500 ? 0.20 : 0.10);
